 ...
 */
static inline int ctrl_msg(usb_dev_handle * device,
			   ...
			   unsigned char *data, size_t length)
{
	...
	return usb_control_msg(device,
			       ...
			       (int)request,
			       (int)value,
			       (int)index, (char *)data, (int)length, 10000);
#endif
}
 ...
 */
static int ezusb_read(usb_dev_handle * device,
		      char *label,
		      ...
		      unsigned short addr, unsigned char *data, size_t len)
{
	...
	if (verbose)
		printf("%s, addr 0x%04x len %4d (0x%04x)\n", label, addr,
		       (int)len, (int)len);
	status =
	    ctrl_msg(device, USB_DIR_IN | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
		     opcode, addr, 0, data, len);
	if (status != (int)len) {
		if (status < 0)
 ...
 */
static int ezusb_write(usb_dev_handle * device,
		       char *label,
		       unsigned char opcode,
		       unsigned short addr,
		       const unsigned char *data, size_t len)
{
	...
	if (verbose)
		printf("%s, addr 0x%04x len %4d (0x%04x)\n", label, addr,
		       (int)len, (int)len);
	status =
	    ctrl_msg(device, USB_DIR_OUT | USB_TYPE_VENDOR | USB_RECIP_DEVICE,
		     opcode, addr, 0, (unsigned char *)data, len);
	if (status != (int)len) {
		if (status < 0)
			printf("%s: %s\n", label, strerror(errno));
		else
 ...
 */
static int ezusb_cpucs(usb_dev_handle * device, unsigned short addr, int doRun)
{
	int status;
 ...
 */
static inline int ezusb_get_eeprom_type(usb_dev_handle * device,
					unsigned char *data)
{
	return ezusb_read(device, "get EEPROM size", GET_EEPROM_SIZE, 0, data,
			  1);
}
		 ...
		 usb_dev_handle * device)
{
	...
	if (rc != 0) {
		printf
		    ("Error locating firmware array 1 in imagic driver file\n");
		return -2;
	}

	while (1) {
		rc = fread(&imagic_firmware_element, 1,
			   sizeof(imagic_firmware_element), image);
		...
		rc = poke(context, imagic_firmware_element.load_addr, 0,
			  imagic_firmware_element.data,
			  imagic_firmware_element.length);
		if (rc < 0)
			return -1;
	}
	...
	if (rc != 0) {
		printf
		    ("Error locating firmware array 2 in imagic driver file\n");
		return -2;
	}
	while (1) {
		rc = fread(&imagic_firmware_element, 1,
...
static int ram_poke(void *context,
		    unsigned short addr,
		    int external, const unsigned char *data, size_t len)
{
	...
	case internal_only:	/* CPU should be stopped */
		if (external) {
			printf
			    ("can't write %d bytes external memory at 0x%04x\n",
			     (int)len, addr);
			return -EINVAL;
		}
		break;
	...
	case skip_external:	/* CPU should be stopped */
		if (external) {
			if (verbose >= 2) {
				printf
				    ("SKIP external RAM, %d bytes at 0x%04x\n",
				     (int)len, addr);
			}
			return 0;
		}
	...
	while ((rc = ezusb_write(ctx->device,
				 ...
				 addr, data, len)) < 0 && retry < RETRY_LIMIT) {
		retry += 1;
	}
	return (rc < 0) ? rc : 0;
}
 ...
 */
int ezusb_load_ram(usb_dev_handle * device, const char *path, int fx2,
		   int stage, Prototype_EzUsb_control_msg uptr)
{
	FILE *image;
...
int ezusb_load_ram_imagic(usb_dev_handle * device, const char *path,
			  Prototype_EzUsb_control_msg uptr)
{
	FILE *image;
	struct ram_poke_context ctx;
	int status;
...
static int eeprom_poke(void *context,
		       unsigned short addr,
		       int external, const unsigned char *data, size_t len)
{
	...
	if ((rc = ezusb_write(ctx->device, "write EEPROM segment header",
			      RW_EEPROM, ctx->ee_addr, header, 4)) < 0)
		return rc;

	/* write code/data */
	if ((rc = ezusb_write(ctx->device, "write EEPROM segment",
			      RW_EEPROM, ctx->ee_addr + 4, data, len)) < 0)
		return rc;
 ...
 */
int ezusb_load_eeprom(usb_dev_handle * dev, const char *path, const char *type,
		      int config)
{
	FILE *image;
	unsigned short cpucs_addr;
	...
	if (strcmp("fx2", type) == 0) {
		...
		ctx.ee_addr = 8;
		config &= 0x4f;
		printf("FX2:  config = 0x%02x, %sconnected, I2C = %d KHz\n",
		       config, (config & 0x40) ? "dis" : "",
		...
		ctx.ee_addr = 9;
		config &= 0x07;
		printf("FX:  config = 0x%02x, %d MHz%s, I2C = %d KHz\n",
		       config,
		       ((config & 0x04) ? 48 : 24),
